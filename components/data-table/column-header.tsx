      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="sm" className="-ml-3 h-8 data-[state=open]:bg-accent">
            <span>{formatCapitalize(title)}</span>
            {column.getIsSorted() === "desc" ? <MoveDown className="ml-2 h-1 w-4" /> : column.getIsSorted() === "asc" ? <MoveUp className="ml-2 h-1 w-4" /> : <ChevronsUpDown className="ml-2 h-4 w-4" />}
          </Button>
        </DropdownMenuTrigger>
